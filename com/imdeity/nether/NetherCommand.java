		...
		if(args.length == 1 && args[0].equalsIgnoreCase("join")){
		  if(player.hasPermission("Deity.nether.override")){
				...
				if(playerHasWaited(player)) {
					...
					player.sendMessage(ChatColor.RED + "[DeityNether] You must wait " + ChatColor.GREEN + DeityNether.PLAYER_JOIN_NETHER_WAIT_MINUTES/60 + ChatColor.RED + " hours before entering the nether again!");
					return true;
				}
			} else {
			...
			moveMain(player);
			player.sendMessage(ChatColor.RED + "[DeityNether] Welcome back to the main world, you still have " + ChatColor.GOLD + DeityNether.TIME_LEFT + ChatColor.RED + " minutes left in the nether.");
			return true;
